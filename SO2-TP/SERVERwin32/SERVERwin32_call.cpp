...
LRESULT CALLBACK TrataEventos(HWND hWnd, UINT messg, WPARAM wParam, LPARAM lParam) {
	switch (messg) {
		...
		case WM_COMMAND:
			switch (wParam) {
			case IDM_Info:
				DialogBox(hInstancia, MAKEINTRESOURCE(IDD_DLG_Info), hWnd, DialogInfo);
				break;
...
//IDD_DLG_Setup
LRESULT CALLBACK DialogSetup(HWND hDlg, UINT messg, WPARAM wParam, LPARAM lParam) {
	...
	switch (messg) {
		...
		case WM_COMMAND:
			if (LOWORD(wParam) == IDOK){
				GetDlgItemText(hDlg, IDC_NUM_JOG, Num_jogadores, 2);
